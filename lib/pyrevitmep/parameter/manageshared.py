...
# WPF sorting types live in WindowsBase; make sure it's loaded for IronPython.
try:
...
class ManageSharedParameter(WPFWindow):
    ...
    @staticmethod
    def sort_datagrid(datagrid, column_index=0, list_sort_direction=None):
        # type: (DataGrid, int, object) -> None
        """Method use to set actual initial sort.
        cf. https://stackoverflow.com/questions/16956251/sort-a-wpf-datagrid-programmatically"""
        if ListSortDirection is None or SortDescription is None:
            return

        if list_sort_direction is None:
            list_sort_direction = ListSortDirection.Ascending
