...
def _default_export_name(selected_types):
    return "NWB_Maintainability_{0}_Schedule_{1}.xlsx".format(_types_slug(selected_types), _timestamp())


def _choose_export_paths(selected_types):
    default_name = _default_export_name(selected_types)
    try:
        xlsx_path = forms.save_file(file_ext="xlsx", default_name=default_name)
    except Exception:
        xlsx_path = ""

    xlsx_path = _safe_str(xlsx_path).strip()
    if not xlsx_path:
        return "", "", True
    ...
    return xlsx_path, csv_path, False
...
def run():
    ...
    xlsx_path, csv_path, cancelled = _choose_export_paths(selected_types)
    if cancelled:
        return
