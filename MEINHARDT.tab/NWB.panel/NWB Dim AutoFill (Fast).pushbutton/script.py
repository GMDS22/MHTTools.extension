...
# OPTIMIZATION: Global caches for this tool
...
def _find_param_target(element, name, target_mode, logger=None):
    ...
    if target_mode == "FamilyOnly":
        if nonshared:
            ...
            result = ([best] if best is not None else [], "family-only", candidates)
            if owner_id:
                _PARAM_RESOLUTION_CACHE[cache_key] = result
            return result
    ...
    if shared:
        ...
        result = ([best] if best is not None else [], "shared-only", candidates)
        if owner_id:
            _PARAM_RESOLUTION_CACHE[cache_key] = result
        return result
    if nonshared:
