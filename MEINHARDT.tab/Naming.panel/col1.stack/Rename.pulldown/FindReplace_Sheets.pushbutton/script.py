...
def get_target_sheets():
    """Use selected sheets, title blocks, or viewports before showing the picker."""
    selected = []
    seen_ids = set()
    ...
    for element_id in uidoc.Selection.GetElementIds():
        ...
        if isinstance(element, ViewSheet):
            sheet = element
        elif hasattr(element, 'SheetId') and element.SheetId != ElementId.InvalidElementId:
            ...
            if isinstance(owner_view, ViewSheet):
                sheet = owner_view

        if isinstance(sheet, ViewSheet) and sheet.Id.IntegerValue not in seen_ids:
            selected.append(sheet)
            seen_ids.add(sheet.Id.IntegerValue)

    if selected:
        return selected

    return get_selected_sheets(given_uidoc=uidoc,
                               title=__title__,
...
class MyWindow(forms.WPFWindow):
    ...
    def __init__(self, xaml_file_name):
        ...
        self.selected_sheets = get_target_sheets()
