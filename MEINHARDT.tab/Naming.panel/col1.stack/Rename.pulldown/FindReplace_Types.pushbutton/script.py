...
class RenameFamilyTypes(BaseRenaming):
    ...
    def get_selected_elements(self):
        ...
        for element_id in uidoc.Selection.GetElementIds():
            ...
            candidate = element if isinstance(element, ElementType) else None

            if candidate is None and element is not None:
                type_id = element.GetTypeId()
                if type_id != ElementId.InvalidElementId:
                    candidate = doc.GetElement(type_id)

            if candidate is not None and type(candidate) in incl_types and candidate.Id.IntegerValue not in seen_ids:
                selected_types.append(candidate)
                seen_ids.add(candidate.Id.IntegerValue)
