...
class LinkedRoomRegionWindow(WPFWindow):
    ...
    def _sync_selected_room_state_from_ui(self):
        ...
        visible_host_ids = set()
        visible_link_pairs = set()
        for item in self._filtered_room_items:
            if item.link_inst is not None:
                visible_link_pairs.add((item.link_inst.Id.IntegerValue, item.room_id))
            else:
                visible_host_ids.add(item.room_id)

        self._selected_host_room_ids.difference_update(visible_host_ids)
        self._selected_link_room_pairs.difference_update(visible_link_pairs)
    ...
    def _clear_selected_rooms(self):
        self._selected_host_room_ids = set()
        self._selected_link_room_pairs = set()
        self._is_syncing_room_selection = True
        try:
            if self.lstRooms is not None:
                self.lstRooms.UnselectAll()
        finally:
            self._is_syncing_room_selection = False
        self._update_summary()
    ...
    def on_clear_rooms(self, sender, args):
...
def _flatten_curve_to_elevation(curve, elevation, z_tol=1e-4):
    ...
    try:
        ...
        dz = float(elevation) - float(p0.Z)
        if abs(dz) <= z_tol:
            return curve

        return curve.CreateTransformed(Transform.CreateTranslation(XYZ(0, 0, dz)))
    except Exception:
        try:
            pts = list(curve.Tessellate())
            if not pts:
                return None
            dz = float(elevation) - float(pts[0].Z)
            return curve.CreateTransformed(Transform.CreateTranslation(XYZ(0, 0, dz)))
        except Exception:
            return None
...
def _curve_collection_to_overlap_segments(curves, min_len=MIN_CURVE_LEN_FT):
    segments = []
    for curve in curves or []:
        if curve is None:
            continue

        if _is_effectively_line(curve):
            try:
                p0 = curve.GetEndPoint(0)
                p1 = curve.GetEndPoint(1)
                if _distance(p0, p1) > min_len:
        ...
        for pt in pts:
            ...
            if _distance(last_pt, pt) <= min_len:
                last_pt = pt
                continue

            try:
                segments.append(Line.CreateBound(last_pt, pt))
            except Exception:
                pass
            last_pt = pt
...
def _collect_selected_room_loops(room_items, target_elevation, tol=0.02):
    ...
    for item in room_items:
        ...
        for location_name in ("Center", "Finish"):
            try:
                boundary_locations.append(getattr(SpatialElementBoundaryLocation, location_name))
            except Exception:
                continue
        if not boundary_locations:
...
def _remove_shared_interior_segments(curves, tol=0.02):
    fragmented = _fragment_line_segments_at_overlaps(curves, tol)
    buckets = {}
...
def _collect_linked_room_pairs_from_refs(refs):
    link_pairs = set()
    rejection_notes = []

    for rf in refs or []:
        try:
            link_inst = doc.GetElement(rf.ElementId)
        except Exception:
            link_inst = None

        if link_inst is None:
            rejection_notes.append("Reference without link instance was skipped.")
            continue
        ...
        if link_doc is None:
            rejection_notes.append("Link '{0}' is unloaded and was skipped.".format(_safe_elem_name(link_inst, "Linked Model")))
            continue

        linked_id = getattr(rf, "LinkedElementId", None)
        if linked_id is None or linked_id == ElementId.InvalidElementId:
            rejection_notes.append("A picked linked element had no valid LinkedElementId and was skipped.")
            continue
        ...
        if linked_elem is None:
            rejection_notes.append("A picked linked element could not be resolved and was skipped.")
            continue

        try:
            is_room = linked_elem.Category is not None and linked_elem.Category.Id.IntegerValue == int(BuiltInCategory.OST_Rooms)
        except Exception:
            is_room = False

        if not is_room:
            rejection_notes.append("Non-room linked element '{0}' was skipped.".format(_safe_elem_name(linked_elem, "Element")))
            continue

        try:
            link_pairs.add((rf.ElementId.IntegerValue, linked_id.IntegerValue))
        except Exception:
            rejection_notes.append("A linked room id could not be serialized and was skipped.")

    return link_pairs, rejection_notes


def _pick_rooms_for_state(state):
    current_host_ids = set(state.get("selected_host_room_ids", []) or [])
    current_link_pairs = set(tuple(pair) for pair in (state.get("selected_link_room_pairs", []) or []))
    new_host_ids = set()
    new_link_pairs = set()
    linked_instances = list(FilteredElementCollector(doc).OfClass(RevitLinkInstance).ToElements())
    prefer_linked_pick = bool(linked_instances)

    try:
        if prefer_linked_pick:
            with forms.WarningBar(title='Select linked rooms and click "Finish"'):
            ...
            if refs:
                extracted_pairs, rejection_notes = _collect_linked_room_pairs_from_refs(refs)
                new_link_pairs.update(extracted_pairs)
    ...
    if new_host_ids or new_link_pairs:
        current_host_ids.update(new_host_ids)
        current_link_pairs.update(new_link_pairs)
        state["selected_host_room_ids"] = sorted(current_host_ids)
        state["selected_link_room_pairs"] = sorted(list(current_link_pairs))
        state["room_filter"] = ""
    return state
...
def _create_filled_regions(view, region_type, loops, hole_map=None, line_style_id=None):
    ...
    if loops and not hole_map:
        ...
        for set_index, boundary_set in enumerate(boundary_sets):
            ...
            # Recover from invalid grouped holes by validating one hole at a time.
...
def _extract_curves_from_loops(curve_loops):
    curves = []
    for curve_loop in curve_loops or []:
        try:
            for curve in curve_loop:
                curves.append(curve)
        except Exception:
...
def _write_operation_log_file(lines):
    ...
    try:
        with open(log_path, "w") as log_file:
            log_file.write("\n".join(lines))
        try:
            latest_log_path = os.path.join(temp_dir, _LATEST_DEBUG_LOG_FILE_NAME)
            with open(latest_log_path, "w") as latest_log_file:
...
def _show_result_alert(lines, log_path=None):
    message_lines = list(lines or [])
    if log_path:
        message_lines.extend(["", "Debug log file:", log_path])
...
def _execute_room_region_operation(view, data):
    if view is None:
        _show_result_alert(["No active view found."])
        return False

    if not data:
        _show_result_alert(["No create request data was provided."])
        return False
